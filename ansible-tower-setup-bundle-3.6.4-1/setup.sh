...
if [ $? -eq 0 ]; then
    ...
    if [ "$(distribution_id)" == "centos" ] || [ "$(distribution_id)" == "rhel" ] || [ "$(distribution_id)" == "ol" ]; then
        ...
        if [ -f "${ANSIBLE_RPM_PATH}" ]; then
            yum install -y ${ANSIBLE_RPM_PATH} ${PYTHON_CRYPTO_RPM_PATH} ${PYTHON_HTTPLIB_RPM_PATH} ${PYTHON_KEYCZAR_RPM_PATH} ${PYTHON_PARAMIKO_RPM_PATH} ${PYTHON_ECDSA_RPM_PATH} ${SSHPASS_RPM_PATH} ${LIBTOMCRYPT_RPM_PATH} ${LIBTOMATH_RPM_PATH} ${PYTHON_JINJA_RPM_PATH} ${PYTHON_PASSLIB_RPM_PATH} ${PYTHON_JMESPATH_RPM_PATH}
        else
